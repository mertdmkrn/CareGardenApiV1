...
namespace CareGardenApiV1.Repository.Concrete
{
    public class CommentRepository : ICommentRepository
    {
        ...
        public async Task<Dictionary<string, dynamic>> GetCommentStatisticsByBusinessId(Guid businessId)
        {
            ...
            retVal.Add("Count", pointList.IsNullOrEmpty() ? 0 : pointList.Count());
            retVal.Add("Average", pointList.IsNullOrEmpty() ? 0 : Math.Round(pointList.Average(), 1));

            foreach (var item in points)
            {
                var point = pointListGroups.FirstOrDefault(x => x.Key == item);
                retVal.TryAdd(item.ToString(), point == null ? 0 : point.Count());
            }
